...
public class TLShape extends TetrisPiece {
    ...
    public boolean locationPreview() {
        ...
        if(collisionCheck(Brick1, Brick2, Brick3,
                           Brick4, Brick5, CollisionType.RIGHT)){
        
            //must restore stored x,y values
            ...
            System.out.println("preview shows Crash");
            return true;
                

        }
        
        //preview was OK, but still need to restore x,y as the move still 
        //might not be valid. This fucntion only knows if the grid space is 
        //ok or not
        Brick1.setBrickX(B1x);
        Brick1.setBrickY(B1y);
        ...
        System.out.println("preview is OK");
        return false;
    }
...
//            TBrick.brickCloneLocation(Brick4, Brick5);
    ...
    public void rLeft() {
        ...
        //check collisionCheck() for hitting game wall
        if(collisionCheck(Brick1, Brick2, Brick3,
                           Brick4, Brick5, CollisionType.LEFT)){
                System.out.println("rLeft() CRASH, move not allowed!");
                valid = false;
                return;
         }
        
        System.out.println("rLeft() Try to Rotate");
        //move from state 0 to state -1
        if(getRotationState() == 0 && valid == true) {
